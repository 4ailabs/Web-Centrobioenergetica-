  ...
  return (
    ...
    >
      <div className="p-3 md:p-6 flex-grow">
        ...
        <h3 className="text-base md:text-2xl font-extrabold text-slate-900 mb-2 md:mb-4 tracking-tight leading-snug product-name">{kit.name}</h3>
        ...
        <div className="flex items-center gap-2 mb-2 md:mb-3">
          ...
          <span className="text-xs text-slate-500">{kitProducts.length} productos</span>
        </div>
        ...
        <div className="space-y-1 md:space-y-3 mt-2 md:mt-4">
          ...
          <ul className="text-xs md:text-base text-slate-800 space-y-0.5 md:space-y-2">
            {kitProducts.slice(0, 4).map(product => (
              <li key={product.id} className="flex items-start gap-1">
            ...
            {kitProducts.length > 4 && (
              <li className="text-slate-500 font-medium text-xs flex items-center gap-1">
                <span className="text-green-500">+</span>
                {kitProducts.length - 4} productos más incluidos
              </li>
            )}
          </ul>
        </div>

        <div className="mt-3 md:mt-4 p-2 md:p-3 bg-white/50 rounded-lg border border-white/20">
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-600 font-medium">Ahorro total:</span>
            <span className="text-green-600 font-bold">{kit.discount}% OFF</span>
          </div>
          <div className="text-xs text-slate-500 mt-1">
            Solución completa para: <span className="font-medium text-slate-700">{kit.problem}</span>
          </div>
        </div>
      </div>
